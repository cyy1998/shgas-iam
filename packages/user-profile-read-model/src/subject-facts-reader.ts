...
export function createSubjectFactsReader(options: CreateSubjectFactsReaderOptions) {
  const profileLoads = new Map<string, Promise<SubjectFactsCacheRecordV1 | null>>();
  ...
  return {
    async read(subjectIdentifier: string): Promise<SubjectFactsSnapshot | null> {
    ...
    async check(input: {
      ...
      if (reloaded === null || reloaded.sourceDirtyVersion !== dirty.dirtyVersion) {
        recordObservation(options.observability, {
          operation: "authorization-freshness",
          outcome: "not-ready",
          durationMs: elapsedMilliseconds(freshnessStartedAt, now()),
...
function parseCachedRecord(input: string | null) {
  ...
  try {
    const parsed = SubjectFactsCacheRecordV1Schema.safeParse(JSON.parse(input));
...
function toSubjectFactsSnapshot(record: SubjectFactsCacheRecordV1): SubjectFactsSnapshot {
  return {
