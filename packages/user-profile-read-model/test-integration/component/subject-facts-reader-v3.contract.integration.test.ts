...
describe("Subject Facts v3 Reader", () => {
  test("uses one strict v3 cached record without querying PostgreSQL", async () => {
    const select = mock(() => {
      throw new Error("a v3 cache hit must not query PostgreSQL");
    });
