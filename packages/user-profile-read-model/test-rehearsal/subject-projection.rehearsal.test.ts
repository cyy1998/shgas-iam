...
import type {
  RedisTestHarness,
  RedisTestScope,
} from "../test-redis/redis-test-harness";
...
import { createPostgresTestHarness } from "../test-postgres/postgres-test-harness";
import { createRedisTestHarness } from "../test-redis/redis-test-harness";
...
test("rehearses Subject Facts prewarm and authorization reads at approximate production volume", async () => {
  const report = await runWithOwnedTestResources(async ({ registerCleanup }) => {
    ...
    expect(coldFacts.every(facts => facts !== null)).toBeTrue();
    const coldObservations = observations;
    expect(observationsFor(coldObservations, "cache-read", "miss"))
      .toHaveLength(COLD_SAMPLE_SIZE);
    expect(observationsFor(coldObservations, "profile-load", "ready"))
      .toHaveLength(COLD_SAMPLE_SIZE);
    ...
    expect(coldFreshness.every(result => result.status === "fresh")).toBeTrue();
    ...
    expect(singleFlightFacts.every(facts => facts !== null)).toBeTrue();
    expect(observationsFor(observations, "profile-load", "ready"))
      .toHaveLength(1);
    ...
    expect(singleFlightWaits.length).toBeGreaterThan(0);
    ...
    expect(prewarmPublished).toBe(SYNTHETIC_USER_COUNT);
    ...
    expect(warmFacts.every(facts => facts !== null)).toBeTrue();
    const warmObservations = observations;
    expect(observationsFor(warmObservations, "cache-read", "hit"))
      .toHaveLength(WARM_SAMPLE_SIZE);
    expect(observationsFor(warmObservations, "profile-load")).toHaveLength(0);
    ...
    expect(warmFreshness.every(result => result.status === "fresh")).toBeTrue();
    const warmFreshnessObservations = observations;

    observations = [];
    expect(await reader.read(syntheticSubjectIdentifier(
      SYNTHETIC_USER_COUNT + 1,
    ))).toBeNull();
    expect(observationsFor(observations, "profile-load", "not-ready"))
      .toHaveLength(1);
