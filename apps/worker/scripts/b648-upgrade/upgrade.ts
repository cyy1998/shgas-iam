...
function currentTarget(row: Row) {
  ...
  const config
    = row.sso_config === null
      ? null
      : normalizeOfflineClientSsoConfig(row.sso_config as Parameters<typeof normalizeOfflineClientSsoConfig>[0]);
  const emptySecret
...
export async function runClientSsoUpgradeInTransaction(
  tx: postgres.TransactionSql,
  options: { mode: "inventory" | "apply" | "verify"; manifest?: ClientSsoUpgradeManifest; all: boolean },
) {
  ...
  const writes: Array<{
    code: string;
    config: OfflineClientSsoConfig | null;
    enabled: boolean;
