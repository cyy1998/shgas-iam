...
import {
  createAdminMutation,
  runAdminSubjectAccessMutation,
} from "@admin-api/services/admin-mutation/admin-mutation";
...
export function createUserService(deps: AdminUserServiceDeps) {
  ...
  async function assertUserActionAllowed(
    ...
    if (authorization?.kind !== "scoped")
      return;
    const openEmploymentOrganizationIds = await repository.getOpenEmploymentOrganizationIdsByUserId(user.id);
  ...
  async function getUserDetailForAdmin(user: Parameters<typeof UserDetailDtoSchema.parse>[0]) {
    ...
    const roleIds = [
      ...new Set([...rolesByEmployment.values()].flatMap(roles => roles.map(role => role.id))),
    ];
    const roleNames = new Map(
      (await deps.roleRepository.getRoleNamesByIds(roleIds)).map(role => [role.roleCode, role.roleName]),
    );
    for (const employment of employments) {
      ...
      employmentDto.roleNames = Object.fromEntries(
        roles.flatMap((role) => {
          const name = roleNames.get(role.roleCode);
          return name === undefined ? [] : [[role.roleCode, name]];
        }),
      );
      employmentDto.privilegeNames = Object.fromEntries(
        privileges.map(p => [p.privilegeCode, p.privilegeName]),
      );
    ...
    userDto.privilegeNames = Object.fromEntries(
      currentEmploymentDtos.flatMap(e => Object.entries(e.privilegeNames)),
    );
    return userDto;
  }
  ...
  async function setUserForAdmin(dto: UserAdminCreateDto, auditContext?: AdminAuditContext) {
    ...
    const disposition
      = (dto.status ?? UserStatus.Enable) === UserStatus.Enable ? "awaiting_publication" : "disabled";
    return await runAdminSubjectAccessMutation(deps.subjectAccessLifecycle, {
      subjectIdentifier,
      disposition,
      mutate: async receipt =>
        await adminMutation.transaction(
          async tx =>
            await tx.subjectAccessMutation.runMutation(
              ...
              async () => {
                ...
                await tx.userProfileInvalidation.recordChanges([{ kind: "user", userId: createdUser.id }]);
                return {
                  changed: true,
                  result: {
  ...
  async function updateUser(
    ...
    const existing
      = authorization?.kind === "scoped" && data.status !== undefined
      ...
      return await adminMutation.locked(
        ...
        async (tx, current) => {
          const mutation = async () => {
            await assertUserActionAllowed(tx.userRepository, current, authorization, authorizationAction);
            const changed
            ...
            if (changed) {
              await tx.userProfileInvalidation.recordChanges([{ kind: "user", userId: current.id }]);
            }
            return {
              changed,
              result: {
          ...
          return await tx.subjectAccessMutation.runMutation(receipt, mutation, result =>
    ...
    if (data.status !== undefined) {
      if (existing === null)
        throw new UserNotFoundError("用户不存在");
      await assertUserActionAllowed(deps.userRepository, existing, authorization, authorizationAction);
      const result = await runAdminSubjectAccessMutation<Awaited<ReturnType<typeof mutate>>>(
        deps.subjectAccessLifecycle,
        {
  ...
  async function updateUserStatus(
    ...
    return await updateUser(username, { status }, auditContext, authorization, "changeStatus");
  }
  ...
  async function resetPasswordByUsername(
    ...
    let sessions: Awaited<ReturnType<NonNullable<typeof deps.resetPasswordSessionEffect>>> | undefined;
    const result = await adminMutation.locked(
      tx => tx.userRepository.lockUserByUsername(username, authorization?.kind === "scoped"),
      () => new UserNotFoundError("用户不存在"),
      async (tx, user) => {
        await assertUserActionAllowed(tx.userRepository, user, authorization, "resetPassword");
        if (
          deps.resetPasswordSessionEffect
          && auditContext?.actorUserId === user.id
          && !auditContext.principalSessionId
        ) {
          throw new BadRequestError("当前管理会话缺失，未重置密码");
        }
        ...
        await tx.auditService.recordAuditLog(
          buildAdminUserAudit(
            userMutationActions.resetPassword.auditAction,
            user,
            {
              passwordReset: true,
              changed: true,
        ...
        if (deps.resetPasswordSessionEffect) {
          tx.afterCommit.required("admin.session_revoke.user", async () => {
            sessions = await deps.resetPasswordSessionEffect!({
              userId: user.id,
    ...
    return { ...result, ...(sessions === undefined ? {} : { sessions }) };
  }
