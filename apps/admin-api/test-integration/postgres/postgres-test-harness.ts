...
const TEST_DATABASE_URL_ENV = "IAM_ADMIN_API_TEST_DATABASE_URL";
...
export async function createAdminApiPostgresTestHarness(): Promise<AdminApiPostgresTestHarness> {
  const databaseUrl = requireDedicatedTestDatabaseUrl();
  const schemaName = `iam_admin_api_${randomUUID().replaceAll("-", "")}`;
  ...
  try {
    ...
    scopedSql = postgres(databaseUrl, {
      connection: {
        application_name: `iam-admin-api-${schemaName}`,
