...
- [ ] Seed 形成 active admin subject、密码、组织/任职/角色、Custom SSO client 与 public OIDC PKCE client，并只返回
  run-scoped 非敏感引用。
- [ ] Redis 不复制 key/serializer/TTL/Lua；seed 不公开 DSL 或通用 fixture interface。
- [ ] `IAM_SSO_INTERNAL_HOST`/`IAM_SSO_EXTERNAL_HOST`、issuer 与 registered redirects 都对应同一个
  `127.0.0.1` 动态 origin。
