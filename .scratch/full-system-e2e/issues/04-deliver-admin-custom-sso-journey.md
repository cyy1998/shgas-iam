...
- [ ] Gateway、Admin、SSO、API、Admin API、PostgreSQL、Redis、Session/Cookie 与 Custom SSO callback 全部真实。
- [ ] 不用 `page.route` 替代 journey 经过的 repo-owned core；只关闭明确不经过的 CAPTCHA/SMS 等第三方边。
- [ ] 失败保存 trace/screenshot/video，再走统一诊断与 cleanup；cleanup failure 非零。
