...
- [x] 测试资源不与普通 namespace-isolated Redis tests 或其他 owner 共享。
- [x] 缺少 `IAM_API_CORE_CLEANUP_TEST_REDIS_URL` 时在连接前明确非零退出；不得回退普通 test URL 或 runtime Redis。
- [x] ACL 禁止 `FLUSHDB`/`FLUSHALL`；sentinel 保持；before/after inventory 精确等于预期 target 删除集合。
- [x] CLI exit code 与人类可读摘要覆盖 dry-run/apply/verify、幂等和失败路径；cleanup failure 非零退出。
