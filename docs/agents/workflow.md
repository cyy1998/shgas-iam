...
`to-tickets` 和 `implement` 是必须由用户手动开启的阶段。调用任一 skill 前，agent 必须停止自动推进，说明即将进入的阶段及范围，并等待用户在当前会话中明确确认；用户直接请求拆票或实现，可以视为对应阶段的确认。上一阶段产物完成、spec 或拆票方案获批、ticket 变为 `ready-for-agent`，都不构成进入下一阶段的授权，两项确认也不得相互继承。

- 进入 `to-tickets` 前，说明拆票所依据的 spec、计划或对话共识；获得确认后才开始拟定拆票方案。该入口确认不能替代 `to-tickets` 内部发布 tickets 前对拆票方案的确认。
- 每次进入 `implement` 前，说明要实施的具体 ticket；快速路径则说明本次改动范围。获得确认后才能认领 ticket 或修改文件。
...
6. 在依赖前沿逐票实施。每次进入 `implement` 前，说明目标 ticket 并等待用户手动确认；确认后先认领，完成针对性验证和双轴评审后生成该票的受评审实现提交。
