...
本手册用于协调 Organization Responsibility V2、全局 User Profile v3、Client Protocol V2、Internal、Custom SSO 与 OIDC
的人工切换。它只编排仓库已有 owner 命令、人工发布动作和可执行 smoke；不会自动 freeze、备份、部署、切流、回滚或
...
- 从第一条 v3 Profile 写入前开始 freeze，直到全部 post-start gate 和 smoke 通过后一次性放流。
- Profile Detail/Search/Subject Facts 与全部搜索入口必须使用同一个固定 v3 candidate；Client Protocol runtime 使用固定 V2
  candidate。两者都不得分 runtime canary、dual-read、fallback、alias 或现场转换旧版本。
...
- 任一阶段失败都保持相关流量关闭并保存安全诊断。跨 PostgreSQL/Redis 的分阶段命令不宣称全局原子；Profile 失败只通过
  既有 repair/retry 边界收敛，client protocol 失败只对同一 manifest forward 重跑，不能报告部分成功。
...
- Profile v3 切换前一致 backup/version，以及只包含已批准 runtime/config 的 rollback version；
...
- Internal User Detail/DSL 等读取 User Profile 的业务流量；
...
freeze 必须从第一条 v3 Profile backfill 写入前生效。仅把 client 置于 Maintenance 不等于完整 identity/config freeze；traffic
...
4. 固定 User Profile v3 candidate、Client Protocol V2 candidate、compatible config rollback version 和 manifest digest。
   candidate 或 manifest 变化即取消本轮并重新审核。
...
证据复核通过后，部署并只启动 Phase 1 固定 digest 的 User Profile v3 Worker consumer；API、OIDC、Custom SSO 与业务流量
仍保持关闭。确认 Worker readiness、队列连接和 schema version 3 后才能进入 Phase 2。Worker 无法启动或 digest 不匹配时保持
freeze 并取消窗口，不得投递 backfill 后再等待一个尚未运行的 consumer。

任一证据不匹配时保持 freeze 并取消窗口；此时尚未写入 v3 Profile，可按批准的普通发布恢复路径退出。

### Phase 2 — 全量 User Profile v3 backfill

通过唯一版本无关入口登记全部 User 的 Backfill dirty fact，并派发既有 rebuild job：

```bash
pnpm --filter @iam/worker user-profile:backfill
```

`enqueued` 只表示投递完成。保持 freeze，等待 Worker 与 dirty/job 收敛；失败或 stale work 只通过
`user-profile:repair`/既有 retry 边界修复，不得跳过 Integrity Violation、手改 Redis/SQL 或把队列暂时为空视为通过。
...
Organization Responsibility Integrity Violation 都阻断后续阶段。此时尚未推进 client epoch，只能在 freeze 中修复并重跑完整
backfill/readiness；不得以部分 Profile 恢复服务。
...
### Phase 7 — 启动其余固定 User Profile v3 / Client Protocol V2 runtime

保留 Phase 1 已运行且 digest 未变的 v3 Worker，人工部署并启动其余固定 candidate。逐一确认 API、Admin API、OIDC Provider、
Admin、SSO、Gateway route，以及全部 readiness/shutdown owner；任何 runtime、composition 或 route 不是固定 candidate，
Worker digest 发生漂移，或出现旧版本 fallback/read-through，立即保持 freeze 并执行 epoch 后 V2-compatible 恢复。

### Phase 8 — 重跑两道 data gate

在已启动 User Profile v3 / Client Protocol V2 runtime 下按 Phase 3 原顺序再次运行 PostgreSQL 与 Redis/Subject Access
gate。任一失败都说明 runtime 启动后
...
- Admin 创建跨树 `head` responsibility，Worker 发布 strict v3 Profile/Subject Facts；
- Internal Detail 与 responsibility target subtree DSL 读取同一发布结果；
- canonical Filter 覆盖用户状态、same-Employment Organization/Position、Role/Privilege、Organization subtree 与
  `not exists`，Internal/Public legacy 返回相同完整 `UserDto`，Delegation 保持独立组合；
...
- Employment Pause 级联暂停 Assignment 后搜索失效，Resume 后重新命中，End 后再次失效；先前 OIDC snapshot 保持不变；
...
| 边界 | 允许动作 | 禁止动作 |
|---|---|---|
| 第一条 v3 Profile 写入前 | 取消窗口，按普通发布路径恢复原 runtime/流量 | 在 backup/candidate 未确认时继续 |
| 已写 v3 Profile、尚未 epoch | 保持 freeze，通过 repair/retry 收敛并重跑两 gate | 只恢复单一存储、跳过 User、以部分 Profile 恢复服务 |
...
| User Profile v3 / Client Protocol V2 runtime、gate 或 smoke 失败 | 保持 freeze；保存诊断；Profile v3 / Client Protocol V2-compatible rollback 或 forward-fix；重跑 Phase 8–9 | 自动放流、以局部 smoke 覆盖失败、回到 Profile 旧版本或 Client Protocol V1 |
...
- backfill `enqueued`、dirty/job 收敛证据、两轮 PostgreSQL/Redis gate 的状态与聚合计数；
