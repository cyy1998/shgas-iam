...
本文面向需要接入上海燃气 IAM SSO 的第三方业务系统，说明业务系统如何通过 IAM 完成统一登录、获取本系统局部会话、读取当前用户信息以及退出登录。
...
4. 业务系统用授权码建立自己的登录态，后续访问业务接口时携带本系统局部会话。
...
| `/sso/token` | GET | 独立应用模式下用授权码换取局部会话 |
| `/sso/logout` | GET | 退出 IAM 全局会话并清理相关局部会话 |
| `/public/user-info` | GET | 获取当前登录用户详情，需要有效会话 |
...
授权码有效期由 IAM API 环境变量 `IAM_API_AUTH_CODE_TTL_SECONDS` 控制，默认部署示例为 300 秒。授权码只用于建立局部会话，不应在前端长期保存。
...
1. 校验响应成功。
2. 将 `sid` 与本系统用户会话绑定，或直接作为本系统局部会话标识保存。
...
- `Authorization` 头当前直接传局部会话值，不使用 `Bearer` 前缀。
- `Client` 请求头必须与会话所属客户端一致。
- 会话失效时 IAM 返回 401，业务系统应清理本地会话并重新发起 `/sso/authorize`。
...
| `token` | 否 | 当前业务系统局部会话。浏览器有 `global_session` Cookie 时可不传；独立应用建议传。 |

IAM 会清理全局会话以及该全局会话下的有效局部会话。对于独立应用模式，IAM 还会向客户端注册的 `logoutEndpoint` 发起后台通知：
...
业务系统收到通知后应删除对应 `sid` 的本地 session，使单点退出在各业务系统间生效。
...
3. 业务系统不要把 `sid` 写入可被脚本读取的持久化存储；优先使用 HttpOnly Cookie。
