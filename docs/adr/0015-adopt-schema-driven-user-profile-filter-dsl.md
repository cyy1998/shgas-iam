...
搜索只读取已发布 Search Document，并返回完整 User Profile Detail。结果根包含所有未删除用户，不隐式排除 Pause 或 Disable；关联任职只包含 Effective Employment。所有有效 Internal Client 共享同一 DSL 能力，不按 client 裁剪字段。Public `/search` 不直接开放 DSL，但与 Internal 使用相同用户集合和执行语义。Privilege Delegation 不进入 Search Document 或 DSL，`/search-with-delegation` 继续在基础搜索之后独立组合实时 delegation 结果。
...
- numeric ID、任职期间和派生索引字段不属于公开搜索结构；这不删除完整 Detail 与旧响应中已有的 ID。Organization Path 使用语义节点和 `distanceToTarget`，`withinSubtreeOf` 只是路径存在条件的简写。
