...
| [docs/adr/0027-own-online-authentication-lifecycle-time-in-redis.md](adr/0027-own-online-authentication-lifecycle-time-in-redis.md) | decision | Current | 2026-09-08 | 2026-10-31 | Spec #115 已实现 Kernel/Grant/协议期限、OIDC store 和取得时有效性；最终核对补齐 Session/Interaction，交付当前 owner 清理与人工维护手册；候选验证见 #120，环境切换未执行。 |
...
| [docs/architecture/architecture-verification.md](architecture/architecture-verification.md) | architecture | Current | 2026-09-08 | 2026-10-31 | 系统关键约束的验证 owner、代表性测试、静态覆盖范围、聚合 Gate（含 Collection Guard）与人工发布证据边界；包含 Admin 最终契约逐项核对、双 readiness、Employment 诊断、离职 Redis/PG 及三条系统旅程的独立证明范围。 |
| [docs/architecture/backend-architecture.md](architecture/backend-architecture.md) | architecture | Current | 2026-09-08 | 2026-10-31 | 后端 ownership、UoW、Profile/Runtime 与分层验证；Admin 全部写入契约、公共模块和锁序已迁移，Internal 委托独立协调，保留跨表乐观及部署边界。 |
...
| [docs/architecture/repository-map.md](architecture/repository-map.md) | architecture | Current | 2026-09-08 | 2026-10-31 | monorepo apps/packages、共享 contracts/domain 职责、User Profile v3 read-model、root-owned E2E workspace、基础设施与生成目录边界。 |
...
| [docs/architecture/testing-architecture.md](architecture/testing-architecture.md) | architecture | Current | 2026-09-08 | 2026-10-31 | Unit/Integration/E2E canonical collections、资源预算与验证契约；含当前契约分类、删除证据、mapper owner 与消费方能力规则；每票与最终聚合入口包含 Collection Guard。 |
...
| [docs/features/oidc/online-auth-redis-time-contract.md](features/oidc/online-auth-redis-time-contract.md) | feature | Current | 2026-09-08 | 2026-10-31 | Spec #115 的 42 条故事、实现与两类自动化 seam 逐项核对；代码验证和人工切换证据分开。 |
...
| [docs/features/sso/authentication-state-extraction.md](features/sso/authentication-state-extraction.md) | feature | Current | 2026-09-08 | 2026-10-31 | Spec #122 全部 50 条故事、最终 owner 与前票证据复用边界；代码候选验证与环境操作分开。 |
...
| [docs/features/sso/subject-access-operation-contract.md](features/sso/subject-access-operation-contract.md) | feature | Current | 2026-09-08 | 2026-10-31 | Spec #128 全部62故事、最终owner、直接行为证据与测试迁移；最终聚合验收由父规格记录。 |
