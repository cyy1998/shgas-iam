...
- **THEN** command SHALL scan users in batches、mark dirty rows and enqueue user-level rebuild jobs
...
- **THEN** command SHALL scan failed 或 stale dirty rows and enqueue rebuild jobs
