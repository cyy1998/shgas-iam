...
- [ ] 2.1 创建 `dev` 环境 manifest，声明 IAM 基础 routes、upstreams、services、plugin-configs、consumers、ssl 和策略模板文件
- [ ] 2.2 创建 `prod` 环境 manifest 示例，使用占位符或环境变量表达生产域名、upstream、证书引用和环境约束
- [ ] 2.3 为所有受仓库管理的 APISIX 对象定义统一 ID、命名规则和 `managed_by=shgas-iam`、`source=repo-manifest` 归属标记
- [ ] 2.4 增加敏感字段约束，确保 manifest 不包含 Admin API key、JWT secret、TLS private key、数据库密码或第三方系统密钥
- [ ] 2.5 确保 manifest 不把第三方业务应用实例作为实时注册事实来源，只保留基础能力、模板、环境约束或非敏感样例
...
- [ ] 3.4 实现 `diff`，展示仓库声明与远端 `source=repo-manifest` 对象之间的新增、修改和删除候选
- [ ] 3.5 实现 `apply --dry-run`，只输出计划变更且不发送写入或删除请求
- [ ] 3.6 实现 `apply`，通过 Admin API 创建或更新 `source=repo-manifest` 对象，并默认不删除远端对象
- [ ] 3.7 实现显式 `--prune` 删除逻辑，仅删除 `source=repo-manifest` 且已从 manifest 移除的对象
- [ ] 3.8 确保 `diff`、`apply` 和 `--prune` 默认避让 `source=dynamic-registry` 对象，并提示其由 IAM 动态注册流程管理
...
- [ ] 5.1 编写 `gateway/apisix/README.md`，说明 manifest 结构、对象命名、环境分层、来源标签和密钥边界
- [ ] 5.2 记录新增/修改 IAM 基础路由、默认策略、策略模板和环境约束的 Git review 流程
- [ ] 5.3 记录 `validate`、`diff`、`apply --dry-run`、`apply` 和 `--prune` 的使用方式
- [ ] 5.4 记录生产发布、回滚、APISIX 远端配置导出和应急恢复流程
- [ ] 5.5 记录第三方业务应用实例由 IAM 动态注册管理，APISIX 对象使用 `source=dynamic-registry`、`app_code` 和 `config_version` 与仓库基线共存

## 6. 验证

- [ ] 6.1 为 manifest 校验逻辑添加单元测试，覆盖合法配置、缺失引用、重复 ID 和敏感字段
- [ ] 6.2 为 diff/apply 计划生成逻辑添加单元测试，覆盖新增、修改、默认不删除、显式 prune 和避让 `source=dynamic-registry`
