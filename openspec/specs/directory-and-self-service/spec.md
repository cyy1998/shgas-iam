...
- public/internal 目录查询: 证据 `apps/api/src/routes/public/public.routes.ts`, `public.handlers.ts`, `apps/api/src/routes/internal/user/user.routes.ts`, `user.handlers.ts`, `apps/api/src/routes/internal/organization/organization.handlers.ts`, `packages/user-profile-read-model/src/user-profile-query.service.ts`, `packages/user-profile-read-model/src/user-profile.repository.ts`, `apps/api/src/services/organization/organization.repository.ts`。状态: 有代码证据。
