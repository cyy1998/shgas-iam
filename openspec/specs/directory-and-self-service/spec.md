...
系统 SHALL 保持现有 public API 用户服务导出函数和业务行为，同时将用户详情聚合、密码规则、手机号绑定校验和权限委托查询聚合拆分为独立 helper。`user.service.ts` SHALL 作为 use-case facade 保留事务边界和对外编排职责。

#### Scenario: 用户详情聚合委托给 helper
