...
- **THEN** 响应 SHALL 使用 `IAM_API_SSO_EXTERNAL_ORIGIN` 拼接 `authorizationEndpoint`、`logoutEndpoint` 和 `thirdPartyOAEndpoint`
- **AND** endpoint path SHALL 继续来自 `IAM_API_AUTHORIZATION_ENDPOINT`、`IAM_API_LOGOUT_ENDPOINT` 和 `IAM_API_THIRDPARTY_OA_ENDPOINT`
...
- **THEN** 响应 SHALL 使用 `IAM_API_SSO_INTERNAL_ORIGIN` 拼接 `authorizationEndpoint`、`logoutEndpoint` 和 `thirdPartyOAEndpoint`
...
- **WHEN** API 服务启动且 `IAM_API_SSO_INTERNAL_ORIGIN` 或 `IAM_API_SSO_EXTERNAL_ORIGIN` 缺失或不是有效 URL
- **THEN** 环境变量校验 SHALL 失败
- **AND** 服务 SHALL NOT 继续启动

#### Scenario: Public origin is normalized before URL composition
- **WHEN** API 使用 `IAM_API_SSO_INTERNAL_ORIGIN` 或 `IAM_API_SSO_EXTERNAL_ORIGIN` 拼接 endpoint URL
