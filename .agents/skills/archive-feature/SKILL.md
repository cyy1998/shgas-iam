...
2. 用 `git branch --show-current` 读取当前分支。只接受形如 `codex/<slug>` 的功能分支；在 detached HEAD、目标分支、
   非 `codex/` 分支或缺少 slug 时停止。不要切换分支。
3. 把 `codex/` 后唯一、非空的单个路径段作为分支 slug；拒绝包含 `/`、`\`、`.` 或 `..` 路径跳转的值。
4. 若用户显式给出 slug，要求它与分支 slug 完全一致；不一致时停止并同时报告两个值。
5. 不扫描多个 `.scratch/` 子目录让用户选择，也不猜测别名。
6. 把源固定为 `.scratch/<slug>`，目标固定为 `.scratch/archived/<slug>`。要求源是现有目录且目标完全不存在；否则
   在任何修改前停止。
7. 要求 `.scratch/archived` 不存在或是现有目录；若该路径被文件占用，停止并报告冲突。
